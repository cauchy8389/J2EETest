package common.concurrency.MatMult;
...
public class MatMultV2 extends RecursiveAction
{
   private final Matrix a, b, c;
   private final int row;

   public MatMultV2(Matrix a, Matrix b, Matrix c)
   {
      this(a, b, c, -1);
   }

   public MatMultV2(Matrix a, Matrix b, Matrix c, int row)
   {
   ...
   public void compute()
   {
      if (row == -1)
      {
         List<MatMultV2> tasks = new ArrayList<>();
         for (int row = 0; row < a.getRows(); row++)
            tasks.add(new MatMultV2(a, b, c, row));
         invokeAll(tasks);
      }
      else
   ...
   public static void main(String[] args)
   {
      ...
      pool.invoke(new MatMultV2(a, b, c));
      dump(c);
   }
}